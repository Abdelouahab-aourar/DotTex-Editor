        ...
        <aside className="w-15 h-full py-3 bg-background border-r border-white flex flex-col space-y-3 items-center">
            {
                navItems.map((item) => {
                    ...
                    return (
                        <div className={`px-2 py-2 ${isSelected ? "border-l-2 border-l-blue-500" : "border-l-2 border-l-transparent"} border-r-2 border-r-transparent flex items-center justify-center`}>
                            <IconTag
                                key={item.id}
                                size={32}
                                color="white"
